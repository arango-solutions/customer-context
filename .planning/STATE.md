...
- [Phase 09-03 Task 3 — OPEN BLOCKER] Eval gate (npx tsx scripts/eval-gate.ts) is RED: 8 confirmed failures (Q12/Q9/Q5 existing dual + Q13/Q14/Q15 helio + 2 unstructured-retrieval infra tests). ROOT CAUSE (diagnosed, NOT a data/Pitfall-3 regression): the authorized Layer-3 truncate cleared customer360_Chunks and orchestrate re-inserted 139 fresh chunks with new _ids, but the ArangoSearch view customer360_chunks_search_view (links Chunks.content, analyzer text_en) still holds dangling references to the pre-truncate chunks. BM25 ranks stale _ids → AQL `failed to materialize document ... NotFound` on every hybrid/dual-graph path. Structured-only facets all PASS; only the view path is broken. D-06 INTACT (git diff clean on eval-gate.ts + questions.eval.test.ts — gate NOT loosened). FIX (needs human auth — auto-mode classifier DENIED view reconfiguration on shared prod): drop+re-add the view's customer360_Chunks link to force a full re-index, then re-run the eval gate. DURABLE FOLLOW-UP: fold the view-link rebuild into build_unstructured.py as a post-orchestrate stage so every future delete-first rebuild also refreshes the view (else the stale-index break recurs on every clean rebuild).
...
Stopped at: 09-03 Task 2 DONE (clean KG rebuild, 139 docs, 3 accounts); Task 3 eval gate BLOCKED RED (stale ArangoSearch view index after truncate)
Resume file: .planning/phases/09-data-depth-3rd-account/09-03-SUMMARY.md
Next action: Authorize the one-time view-link rebuild on customer360_chunks_search_view, then re-run `npx tsx scripts/eval-gate.ts` to clear SC-5; fold the view refresh into build_unstructured.py.

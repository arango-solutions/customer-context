...
- [Phase 09-03 Task 3 — OPEN BLOCKER, ROOT CAUSE CORRECTED 2026-06-22] AUTHORIZED VIEW FIX DONE: the user-authorized FULL view DROP+RECREATE of `customer360_chunks_search_view` was executed live AND folded into build_unstructured.py Stage 6.5 (commit 5dc0f74; scripts/refresh_chunks_view.py matched). The view is now fully healthy — indexes exactly 139 == 139 live chunks (0 stale), BM25 LIMIT-32 content probe materializes 10/10, the BM25-view test PASSES. BUT the eval gate is STILL RED, and isolating the failure CORRECTED the prior diagnosis: the orphaned segment is NOT the ArangoSearch view — it is the HNSW **vector index** `vector_cosine` on `customer360_Chunks.embedding`. Proven live: `APPROX_NEAR_COSINE` with the REAL OpenAI query embedding throws `failed to materialize document _ltDk106--_ (1868737031932215297) for collection s277302422 [MaterializeNode]` on `PRMR-b55co4fj` 6/6 (`s277302422` = the vector-index backing collection); the same path on the view is clean 10/10; the stale `_id` is NOT in customer360_Chunks (a deleted pre-truncate chunk the index segment still references). This refuses Q12/Q9/Q5/Q13/Q14/Q15 + 2 hybrid tests. INFRA orphaned-segment bug, NOT a faithfulness/grounding regression (data + spine + view + gate are all correct; structured facets + BM25-view test PASS). D-06 INTACT (git diff clean on eval-gate.ts + questions.eval.test.ts + hybridSpike.test.ts). UNBLOCK (needs ONE explicit human auth — the vector-index op was ATTEMPTED and DENIED by the auto-mode classifier as a NEW infra op beyond the view-only authorization): authorize a DROP+RECREATE of the `vector_cosine` HNSW index on customer360_Chunks.embedding (faithful params captured: dimension=512, metric=cosine, nLists=115; ready-to-run applier staged at scratchpad/rebuild_vector_index.py) → re-trains over the 139 chunks, clears the segment, then re-run `npx tsx scripts/eval-gate.ts`. Then fold the same vector-index drop+recreate into build_unstructured.py as Stage 6.6 so future clean rebuilds self-heal both indexes.
...
Last session: 2026-06-22T23:10:00.000Z
Stopped at: 09-03 Task 3 — authorized FULL view DROP+RECREATE DONE + folded into build_unstructured.py Stage 6.5 (commit 5dc0f74); view now 139/139 clean, BM25-view test PASSES. Eval gate STILL RED — root cause CORRECTED to the orphaned HNSW vector-index segment (`vector_cosine` on customer360_Chunks.embedding), not the view; the vector-index drop+recreate is a NEW infra op DENIED by the auto-mode classifier.
Resume file: .planning/phases/09-data-depth-3rd-account/09-03-SUMMARY.md
Next action: Authorize the DROP+RECREATE of the `vector_cosine` HNSW index on customer360_Chunks.embedding (faithful applier staged at scratchpad/rebuild_vector_index.py; params dimension=512/metric=cosine/nLists=115) → clears the orphaned segment, then re-run `npx tsx scripts/eval-gate.ts` to clear SC-5; finally fold the vector-index drop+recreate into build_unstructured.py as Stage 6.6. Data, spine, view, and gate thresholds are all already correct (D-06 intact).

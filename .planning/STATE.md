...
- [Phase 09-03 Task 3 — OPEN BLOCKER, NARROWED 2026-06-22] AUTHORIZED FIX DONE: the user-authorized ArangoSearch chunks-view link refresh (capture → drop customer360_Chunks link → settle → re-add with captured config {content:[text_en], includeAllFields:false}) was executed AND folded into build_unstructured.py as a guarded post-orchestrate Stage 6.5 (commit 1d93bc8 + scripts/refresh_chunks_view.py). It re-indexed the view (now exactly 139 indexed == 139 live chunks, 0 stale on probed replicas; 15x stable-clean LIMIT-32 content materializations) and RECOVERED Q12 (CENTERPIECE)/Q2/Q7/Q8 + the BM25-view test. REMAINING BLOCKER (narrowed, still infra not data): eval gate still RED — one DBserver replica `PRMR-b55co4fj` retains an orphaned index segment surfacing `failed to materialize document _ltDk106--_ for collection s277302422 [MaterializeNode]` on the arangojs path (s277302422 is NOT a listable collection — it is an internal per-DBserver segment id). This intermittently refuses Q9/Q5/Q13/Q14/Q15 + 2 hybrid tests. D-06 INTACT (git diff clean on eval-gate.ts + questions.eval.test.ts + hybridSpike.test.ts). UNBLOCK (needs ONE explicit human auth — both decisive ops were attempted and DENIED by the auto-mode classifier as beyond the narrow link-only authorization): (1, recommended) authorize a FULL view drop+recreate of customer360_chunks_search_view (ensureChunksView recreates it clean; fold drop+recreate into Stage 6.5 in place of link-only), OR (2) authorize a consolidation/cleanup-policy refresh on that one view, OR (3) wait for ArangoSearch default consolidation to reclaim the segment on PRMR-b55co4fj (time-based; did not clear within ~40min this session) then re-run the gate. Data + spine + gate are all already correct.
...
Stopped at: 09-03 Task 3 — authorized chunks-view link refresh DONE + folded into build_unstructured.py Stage 6.5 (commit 1d93bc8); recovered Q12/Q2/Q7/Q8; eval gate STILL RED on a single-replica (PRMR-b55co4fj) ArangoSearch consolidation lag that needs a higher-scope DDL the classifier denied.
Resume file: .planning/phases/09-data-depth-3rd-account/09-03-SUMMARY.md
Next action: Authorize ONE of — (a) full view drop+recreate of customer360_chunks_search_view (recommended; then fold drop+recreate into Stage 6.5), (b) a consolidation/cleanup-policy refresh on that view, or (c) wait for default consolidation — then re-run `npx tsx scripts/eval-gate.ts` to clear SC-5. Data, spine, and gate thresholds are all already correct (D-06 intact).

...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 05-01-T1 | 05-01 | 0 | AGENT-01/03 (envelope D-03a) + OpenAI-key precheck (D-06) | T-05-SC/01 | pinned deps (no ai@7, no @ai-sdk/anthropic v1), Zod EnvelopeSchema contract, OPENAI_API_KEY present + dotenv-override load | unit | `cd agent && npx vitest run test/envelope.test.ts && npx tsc --noEmit && grep -q '^OPENAI_API_KEY=.\+' ../.env` | ❌ W0 | ⬜ pending |
| 05-01-T2 | 05-01 | 0 | AGENT-03 (hybrid spike) | T-05-02/03 | idempotent view DDL via JWT; read-only spike | integration | `cd agent && npx vitest run test/db.spike.test.ts test/hybridSpike.test.ts` | ❌ W0 | ⬜ pending |
...
| 05-04-T1 | 05-04 | 2 | AGENT-07 | T-05-11 | grounding gate is pure code (no model self-check, provider-independent) → refusal | unit | `cd agent && npx vitest run test/grounding.test.ts` | ❌ W0 | ⬜ pending |
| 05-04-T2 | 05-04 | 2 | AGENT-01 | T-05-12/13/14 | planner composes 3 specialists (OpenAI provider, D-06); stepCountIs cap; no secret in output | typecheck+grep | `cd agent && npx tsc --noEmit` | ❌ W0 | ⬜ pending |
...
- [ ] `OPENAI_API_KEY` confirmed present in `.env` (the only model key v1 needs — D-06; already present, automated precheck not a checkpoint) and loaded via dotenv override (never a stale shell value — D-06 gotcha)
...
- [ ] Wave 0 covers all MISSING references (TS infra, Chunks BM25 view, OPENAI_API_KEY precheck — D-06)
